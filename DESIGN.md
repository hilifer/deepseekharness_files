...
### netns 档【并没有】封住同僚实例端口（实测结论，纠正上一稿）

上一稿在这里写过「pasta 必须带 `--no-splice`，加上就封住了」。**那个论断是错的**，
CI 已经证伪：GH runner 上 pasta 正常工作（netns 建起来了、pid ns 隔离、宿主回环
上确实没有 dsh 端口），**带着 `--no-splice` 之后，沙箱内连 `127.0.0.1:13102`
仍然拿到 HTTP 200**。

原因是我把这个参数的作用想错了：`--no-splice` 只决定本地连接**走不走** pasta 的
TCP 栈（splice 是绕过栈的快路径），不决定**能不能到达宿主**。pasta 的设计目标
就是把宿主的网络连通性带进命名空间，宿主回环也在其中。
...
| bwrap + DSH_NETNS=1 | **✘ 仍可达** —— pasta 会把宿主回环带进来 |
| bwrap 默认 | ✘ 直接共享宿主网络 |

**bwrap 档目前在这一项上不满足「个体不能访问其他空间的数据」**，开不开
`DSH_NETNS` 都一样。`preflight` 会如实判红，不再有假绿掩盖。

可能的出路（都还没验证，别当结论）：
- 在 pasta 的命名空间内加一条 netfilter 规则，封掉实例端口段——需要确认那个
  命名空间里拿不拿得到 CAP_NET_ADMIN 与 iptables；
- 放弃 pasta，用纯 `unshare -n` 的空命名空间，出网另想办法（实例要连模型 API）；
- 干脆把 bwrap 排在 landlock 之后，让能封住这条路的档优先。
