...
### bwrap 封不住同僚实例端口，因此排到了 landlock 之后（CI 实测结论）

先纠正两稿之前的错误论断：那时写「pasta 缺 `--no-splice`，补上就封住了」。
**是错的**，CI 证伪。`--no-splice` 只决定本地连接走不走 pasta 的 TCP 栈，
不决定能不能到达宿主。

然后是根因。之前判断 netns 是否生效靠回显 `DSH_NETNS` 环境变量——那只能说明
「请求过」，不能说明「套上了」。加了实测项之后，CI 给出确凿数据：

```
✅ 网络命名空间   与宿主不同（沙箱 net:[4026532332] / 宿主 net:[4026531833]）
   沙箱内网卡     lo eth0
❌ 其他员工的 dsh 实例   HTTP 200 可达
```

**命名空间确实换了，同僚端口照样连得到。** 原因是 pasta 的设计目的就是把宿主的
网络连通性带进命名空间——宿主回环也在其中。这不是配置错误，是这个工具的定位。

于是四档在这一项上的真实状态：

| 档 | 同僚实例端口 | 靠什么封 |
|---|---|---|
| container | ✔ | 独立 netns，天然不可达 |
| landlock | ✔ | Landlock TCP 连接端口白名单（ABI v4+） |
| uid | ✔ | netfilter owner 匹配 |
| **bwrap** | **✘** | 无 —— 开不开 `DSH_NETNS` 都一样 |

这条路不经过文件系统，DAC 与挂载隔离都拦不住；能走通就等于「个体不能访问
其他空间的数据」没达成。所以**择优顺序改为 `container → landlock → bwrap → uid → none`**。

代价如实记，不粉饰：landlock 越界返回 EACCES 会泄露路径存在性，且没有 pid ns；
bwrap 在这两点上更强。两害相权取「数据读不到」优先于「路径名猜不到」——
前者是硬要求，后者是信息泄露。内核 <5.13 没有 landlock 时，bwrap 仍是次优选择，
那时 preflight 会在这一项上如实判红。
