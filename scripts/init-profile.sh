...
# 6) dshmarket 社区插件市场（bundle 插件，从 npm 安装）
if [ -f "$PROFILE_DIR/package.json" ]; then
  export PATH="$DSH_ROOT/node/bin:$PATH"
  if ! grep -q '"dshmarket"' "$PROFILE_DIR/package.json"; then
    (cd "$PROFILE_DIR" && pnpm add dshmarket >/dev/null 2>&1) || \
      echo "警告: pnpm 安装 dshmarket 失败，插件市场可能不可用" >&2
  fi
  python3 - "$PROFILE_DIR/package.json" <<'PY'
import json, sys
p = sys.argv[1]
d = json.load(open(p))
bundles = d.setdefault('dsh', {}).setdefault('profile', {}).setdefault('bundles', [])
if 'dshmarket' not in bundles:
    bundles.append('dshmarket')
    with open(p, 'w') as f:
        json.dump(d, f, ensure_ascii=False, indent=2)
        f.write('\n')
    print(f"bundles 已加 dshmarket: {p}")
PY
fi
